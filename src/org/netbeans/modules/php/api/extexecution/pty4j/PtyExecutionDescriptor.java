 ...
 * @author bogdan
 */
public class PtyExecutionDescriptor {

    private static final Logger LOGGER = Logger.getLogger(PtyExecutionDescriptor.class.getName());
     ...
     */
    public PtyExecutionDescriptor() {
        this(new DescriptorData());
    }

    private PtyExecutionDescriptor(DescriptorData data) {
    ...
    /**
     * Returns a descriptor with configured <i>custom</i> io. When configured to
     * <code>null</code> it means that client is fine with infrustructure
     * provided io (visible as tab in output pane).
     * <p>
     * If configured value is not <code>null</code> values configured via
     * methods
     * {@link #controllable(boolean)}, {@link #rerunCondition(RerunCondition)}
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor inputOutput(@NullAllowed InputOutput io) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.inputOutput(io));
    }
     ...
     * <code>true</code> the control buttons (rerun, stop) will be available io
     * tab created by {@link ExecutionService}.
     * <p>
     * Note that this property has no meaning when custom io is used (see
     * {@link #inputOutput(org.openide.windows.InputOutput)}).
     * <p>
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor controllable(boolean controllable) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.controllable(controllable));
    }
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor frontWindow(boolean frontWindow) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.frontWindow(frontWindow));
    }
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor inputVisible(boolean inputVisible) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.inputVisible(inputVisible));
    }
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor showProgress(boolean showProgress) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.showProgress(showProgress));
    }
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor showSuspended(boolean showSuspended) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.showSuspended(showSuspended));
    }
    ...
    /**
     * Returns a descriptor with configured no reset flag. When configured value
     * is <code>true</code> the output window won't be cleared before the
     * execution. <i>Valid only for custom {@link InputOutput} configured via
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor noReset(boolean noReset) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.noReset(noReset));
    }
     ...
     * @return descriptor with configured flag indicating line based standard
     * output
     * @see
     * #outProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory)
     */
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor outLineBased(boolean outLineBased) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.outLineBased(outLineBased));
    }
     ...
     * error output
     * @see
     * #errProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory)
     */
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor errLineBased(boolean errLineBased) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.errLineBased(errLineBased));
    }
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor frontWindowOnError(boolean frontWindowOnError) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.frontWindowOnError(frontWindowOnError));
    }
     ...
     * <p>
     * Note that {@link ExecutionService} automatically uses the printing
     ...
     * (in case {@link #outLineBased(boolean)} is configured to
     * <code>true</code>) if there is no configured factory.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
     * All other properties of the returned descriptor are inherited from
     ...
     * @return new descriptor with configured factory for additional processor
     * to use for standard output
     * @deprecated use
     * {@link #outProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory2)}
     */
    @Deprecated
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor outProcessorFactory(@NullAllowed InputProcessorFactory outProcessorFactory) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.outProcessorFactory(outProcessorFactory));
    }
     ...
     * Returns a descriptor with configured factory for standard output
     * processor. The factory is used by {@link ExecutionService} to create
     * additional processor for standard output. <i>The configured value will be
     * ignored if you previously configured processor via deprecated
     * {@link #outProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory)}.</i>
     * <p>
     * Note that {@link ExecutionService} automatically uses the printing
     * processor created by
     * {@link org.netbeans.api.extexecution.print.InputProcessors#printing(org.openide.windows.OutputWriter, org.netbeans.api.extexecution.print.LineConvertor, boolean)}
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor outProcessorFactory(@NullAllowed InputProcessorFactory2 outProcessorFactory) {
     ...
     * or
     * {@link org.netbeans.api.extexecution.print.LineProcessors#printing(org.openide.windows.OutputWriter, org.netbeans.api.extexecution.print.LineConvertor, boolean)}
     * (in case {@link #errLineBased(boolean)} is configured to
     * <code>true</code>) if there is no configured factory.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
     ...
     * <code>null</code> allowed
     * @return new descriptor with configured factory for additional processor
     * to use for standard error output
     * @deprecated use
     * {@link #errProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory2)}
     */
    @Deprecated
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor errProcessorFactory(@NullAllowed InputProcessorFactory errProcessorFactory) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.errProcessorFactory(errProcessorFactory));
    }
     ...
     * Returns a descriptor with configured factory for standard error output
     * processor. The factory is used by {@link ExecutionService} to create
     * additional processor for standard error output. <i>The configured value
     * will be ignored if you previously configured processor via deprecated
     * {@link #errProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory)}.</i>
     * <p>
     * Note that {@link ExecutionService} automatically uses the printing
     * processor created by
     * {@link org.netbeans.api.extexecution.print.InputProcessors#printing(org.openide.windows.OutputWriter, org.netbeans.api.extexecution.print.LineConvertor, boolean)}
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor errProcessorFactory(@NullAllowed InputProcessorFactory2 errProcessorFactory) {
     ...
     * <p>
     * Note that {@link ExecutionService} always uses the printing processor for
     * the standard output. Convertor created by the returned factory will be
     * passed to this default printing processor. See
     * {@link #outProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory)}
     * too.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
     ...
     * <code>null</code> allowed
     * @return new descriptor with configured factory for converter for standard
     * output
     */
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor outConvertorFactory(@NullAllowed LineConvertorFactory convertorFactory) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.outConvertorFactory(convertorFactory));
    }
     ...
     * <p>
     * Note that {@link ExecutionService} always uses the printing processor for
     * the standard error output. Convertor created by the returned factory will
     * be passed to this default printing processor. See
     * {@link #errProcessorFactory(org.netbeans.api.extexecution.PtyExecutionDescriptor.InputProcessorFactory)}
     * too.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor errConvertorFactory(@NullAllowed LineConvertorFactory convertorFactory) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.errConvertorFactory(convertorFactory));
    }
     ...
     * runnable is executed <i>before</i> the external execution itself (when
     * invoked by {@link ExecutionService#run()}).
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor preExecution(@NullAllowed Runnable preExecution) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.preExecution(preExecution));
    }
     ...
     * runnable is executed <i>after</i> the external execution itself (when
     * invoked by {@link ExecutionService#run()}).
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor postExecution(@NullAllowed Runnable postExecution) {
     ...
     * the execution, <code>null</code> allowed
     * @return new descriptor with configured post execution callback
     * @since 1.61
     */
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor postExecution(@NullAllowed Consumer<Integer> postExecution) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.postExecution(postExecution));
    }
    ...
    /**
     * Returns a descriptor with configured rerun condition. The condition is
     * used by {@link ExecutionService} to control the possibility of the rerun
     * action.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
     ...
     * @see
     * #rerunCallback(org.netbeans.api.extexecution.PtyExecutionDescriptor.RerunCallback)
     */
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor rerunCondition(@NullAllowed PtyExecutionDescriptor.RerunCondition rerunCondition) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.rerunCondition(rerunCondition));
    }
    ...
    /**
     * Returns a descriptor with configured rerun callback. The callback is
     * invoked when the execution is triggered by the rerun action.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
     * All other properties of the returned descriptor are inherited from
     ...
     * @see
     * #rerunCondition(org.netbeans.api.extexecution.PtyExecutionDescriptor.RerunCondition)
     * @since 1.46
     */
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor rerunCallback(@NullAllowed PtyExecutionDescriptor.RerunCallback rerunCallback) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.rerunCallback(rerunCallback));
    }
    ...
    /**
     * Returns a descriptor with configured options path. If configured value is
     * not <code>null</code> the {@link ExecutionService} will display the
     * button in the output tab displaying the proper options when pressed.
     * <i>For this to work there has to be a {@link OptionOpenHandler} in the
     * system. Otherwise the options button won't be displayed.</i>
     * <p>
     * Note that this property has no meaning when custom io is used (see
     * {@link #inputOutput(org.openide.windows.InputOutput)}).
     * <p>
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor optionsPath(@NullAllowed String optionsPath) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.optionsPath(optionsPath));
    }
    ...
    /**
     * Returns a descriptor with configured charset. If configured value is not
     * <code>null</code> the {@link ExecutionService} will use the given charset
     * to decode the process streams. When <code>null</code> the platform
     * default will be used.
     * <p>
     * Note that in the most common scenario of execution of OS native process
     * you shouldn't need to set the charset. The platform default (which is the
     * default used) is just the right choice.
     * <p>
     * The default (not configured) value is <code>null</code>.
     * <p>
     * All other properties of the returned descriptor are inherited from
    ...
    @NonNull
    @CheckReturnValue
    public PtyExecutionDescriptor charset(@NullAllowed Charset charset) {
        DescriptorData data = new DescriptorData(this);
        return new PtyExecutionDescriptor(data.charset(charset));
    }
    ...
    public interface RerunCondition {
         ...
         *
         * @param listener listener that will listen for changes in rerun
         * possibility
         */
        ...
        /**
         * Returns <code>true</code> if it is possible to execute the action
         * again.
         *
         * @return <code>true</code> if it is possible to execute the action
         * again
         */
    ...
    public interface InputProcessorFactory {
         ...
         *
         * @param defaultProcessor default processor created by infrastructure
         * that is printing chars to the output window
    ...
    private static final class DescriptorData {
        ...
        private PtyExecutionDescriptor.RerunCondition rerunCondition;

        private PtyExecutionDescriptor.RerunCallback rerunCallback;
        ...
        public DescriptorData(PtyExecutionDescriptor descriptor) {
        ...
        public DescriptorData rerunCondition(PtyExecutionDescriptor.RerunCondition rerunCondition) {
            this.rerunCondition = rerunCondition;
            return this;
        }

        public DescriptorData rerunCallback(PtyExecutionDescriptor.RerunCallback rerunCallback) {
